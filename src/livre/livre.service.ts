...
export class livreService {
  ...
  async create(createLivreDto: CreateLivreDto) {
    ...
    const livreWhitLecteur = await this.findOne(createdLivre[0].id);
  ...
  async findOne(id: string) {
    ...
    const data = {
      ...
      lecteur: actorsToFilmsQuery.map((lecteur) => {
        return {
          id: lecteur.livre?.id,
          name: lecteur.livre?.name,
          born: lecteur.livre?.born,
          height: lecteur.livre?.height,
          photo: lecteur.livre?.photo,
          nationality: lecteur.livre?.nationality,
        };
      }),
    };
    return data;
